...
from fastapi.responses import JSONResponse, HTMLResponse, PlainTextResponse, StreamingResponse, RedirectResponse
...
async def _run_scan_async(scan_id: str, req: ScanRequest):
    ...
    try:
        ...
        json_path, html_path = core.save_report(
            result,
            cfg.get("report", {}).get("out", "reports"),
            cfg.get("report", {}),
        )
...
# NEW: simple link helper so the frontend can show "open in new tab" URLs
@app.get("/scan/{scan_id}/report.links")
async def report_links(scan_id: str):
    st = RUNS.get(scan_id)
    if not st:
        raise HTTPException(status_code=404, detail="scan not found")
    base = f"/scan/{scan_id}"
...
# OPTIONAL: redirect shortcuts for nice URLs (open in browser)
@app.get("/scan/{scan_id}/open/html")
async def open_html(scan_id: str):
    return RedirectResponse(url=f"/scan/{scan_id}/report.html")

@app.get("/scan/{scan_id}/open/json")
async def open_json(scan_id: str):
    return RedirectResponse(url=f"/scan/{scan_id}/report.json")
